...
소프트웨어 프로젝트를 진행하다 보면 코드 곳곳에 나중에 처리해야 할 작업을 의미하는 TODO 주석이 남게 된다.  
작은 프로젝트에서는 눈으로 직접 확인할 수 있지만 파일 수가 늘어나면 어떤 TODO가 어디에 얼마나 남아 있는지 파악하기가 점점 어려워진다.

이 실습에서는 특정 디렉터리에 포함된 여러 텍스트 파일을 분석하여 코드에 남아 있는 TODO 항목을 체계적으로 추출하고 정해진 형식의 리포트 문자열을 생성하는 `Todo Analyzer`를 구현한다.  
이 과정에서 파일 입출력과 문자열 파싱, 상태 기반 처리, 결과 정규화 및 정렬 같은 여러 처리 과정을 하나의 프로그램으로 엮는 경험을 한다.
...
java-labs/
 └─ 03-todo-analyzer/
    └─ src/main/java/com/example/todoanalyzer/Main.java
```

6. 검증용 `Main` 클래스는 수정하지 않는다.
7. 같은 패키지에 `TodoAnalyzer` 클래스를 정의한다.
